---
type: promotion-review
packet: "[[learning/packets/architecture-of-original-thought/packet]]"
status: promoted
reviewed: 2026-06-23
promoted: 2026-06-23
---

# Promotion Review

## Gate Check

- [x] Packet export exists.
- [x] Source record exists.
- [x] Human-contact artifact exists.
- [x] Draft synthesis exists.
- [x] Candidate concepts drafted.
- [x] Suggested patches drafted.

## Proposed Promotion

- [x] Promote [[learning/packets/architecture-of-original-thought/drafts/synthesis]] to `wiki/synthesis/original-thought-as-active-reconstruction.md`.

## Candidate Concept Promotions

- [x] First-Principles Reasoning -> `wiki/concepts/first-principles-reasoning.md`
- [x] Desirable Difficulties -> `wiki/concepts/desirable-difficulties.md`
- [ ] Active Reconstruction -> `wiki/concepts/active-reconstruction.md`
- [ ] Illusion of Competence -> `wiki/concepts/illusion-of-competence.md`
- [ ] Cognitive Offloading -> `wiki/concepts/cognitive-offloading.md`

## Existing Concept Patches

- [x] Apply patch to [[wiki/synthesis/llm-wiki-pattern]].
- [x] Apply patch to [[wiki/synthesis/digital-garden-ethos]].
- [x] Apply patch to [[wiki/concepts/digital-garden]].

## Decision Notes

This packet may be especially important for the protocol because it tests whether AI-assisted learning should explicitly preserve desirable difficulty, active recall, and first-principles reconstruction instead of optimizing only for fast synthesis.

The raw export did not satisfy the strict packet export contract. Local repair is recorded in [[learning/packets/architecture-of-original-thought/artifacts/import-repair]].

Human contact was provided on 2026-06-23 through listening and reflection. The reflection emphasizes spaced repetition anxiety, the problem of deciding what is worth remembering, learning as retrieval rather than input, the illusion of competence, and the risk of delegated understanding.

## Promotion Outcome

Promoted on 2026-06-23.

Created trusted notes:

- [[wiki/synthesis/original-thought-as-active-reconstruction]]
- [[wiki/concepts/first-principles-reasoning]]
- [[wiki/concepts/desirable-difficulties]]

Applied approved patches:

- [[wiki/synthesis/llm-wiki-pattern]]
- [[wiki/synthesis/digital-garden-ethos]]
- [[wiki/concepts/digital-garden]]

Deferred candidates:

- Active Reconstruction
- Illusion of Competence
- Cognitive Offloading
- Bloom's Taxonomy
- Levels of Reading
